...
// * 1.0.0 Итак, представим, что у нас есть задача, чтобы при загрузке компонента формы фокус устанавливался на поле ввода e-mail. И это отличный пример задачи, когда нам пригодятся рефы.
// ? ref — это ссылка на элемент или компонент в DOM-дереве (уже в отрендеренном интерфейсе на странице). И так как этот элемент уже существует на странице (не где-то в VirtualDOM), то можно использовать метод focus.
