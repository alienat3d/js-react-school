...
// 1.3.2 Добавим дополнительный стейт charEnded, который будет иметь значение false до тех пор, пока персонажи в базе данных не кончатся. ↓
class CharList extends Component {
  ...
  */
  ...
  // * 1.3.0 Необходимо также продумать поведение, на случай, если персонажи в базе данных подошли к концу. Например, если пользователь много раз кликал подгружать новых и дошёл до самого конца списка. 
  // 1.3.1 Итак, мы можем проверить newCharList на то, что там пустой массив (а так и происходит, благодаря бэкенду Marvel API), то мы можем удалить кнопку, тем самым предупредив возможные ошибки и сэкономив пользователю лишний клик. ↑
  // 1.3.3 Напишем проверку, что у нас пришло меньше, чем 9 элементов, а это значит база данных подошла к концу и в следующий раз грузить будет нечего. И также запишем новый стейт, который удалит кнопку, если он будет в значении true. Для этого понадобится новая переменная в значении false. А также условие, в котором укажем, что если у нас в newCharList пришло меньше 9 элементов, то ended меняет значение на true. Также записываем значение переменной значением в стейт «charEnded». ↓
  ...
  // 1.3.4 Вытащим из стейта новое состояние charEnded и добавим кнопке ещё один атрибут style, в котором мы будем динамически формировать новый объект. Там будет находится свойство "display", значение которого будет формироваться исходя из ответа на тернарник: если charEnded - true, то значение будет 'none', а если 'false', то 'block'.
  render() {
