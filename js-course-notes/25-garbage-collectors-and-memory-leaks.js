...
// * 5-ый случай: ссылки на DOM-элементы. Часто мы сохраняем ссылки на элементы в переменных JS. Одновременно с этим они существуют и в структуре, под названием DOM. Т.е. представление нашей вёрстки в виде объекта. Когда мы удаляем объекты из DOM-дерева, то ссылка на него может оставаться в переменной и этот объект так и будет "висеть", потребляя оперативную память.
...
// Теперь у нас будет создан новый <div>, который будет помещён в переменную div, которая вернётся из функции.
// Если мы создадим новую переменную, то создать мы можем при помощи этой функции. Теперь в переменной testDiv лежит div с id 'test'
const testDiv = createElement();
// Дальше помещаем этот div на страницу.
document.body.append('testDiv');
// И в какой-то момент нужно будет удалить этот блок. Для этого у нас есть функция. И в ней будет использоваться не просто ссылка на элемент, а мы её будем удалять
function deleteElement() {
  document.body.removeChild(document.getElementById('test'));
}
deleteElement();
// Функция deleteElement ориентируется не на какую-то, созданную нами, переменную внутри кода, а именно на DOM. Она находит уникальный id и по нему удаляет этот элемент со страницы. Как это происходит, например, в блокировщиках рекламы, где программа находит определённый CSS-класс и другие селекторы и удаляет их со страницы.
// ? Но тут и появляется проблема, т.к. здесь эта функция удаляет элемент из DOM-дерева, из вёрстки, а в JS этот блок останется. Ведь переменная testDiv сохранилась, она ссылается на тот div, что был создан внутри функции createElement и потому образуется небольшая утечка памяти. И самый простой способ это исправить - это изначально поместить переменную в такие условия, что после создания она будет удалена.
function createElement() {
  const div = document.createElement('div');
  div.id = 'test';
...
createElement();
// Теперь после отработки функции createElement, всё её содержимое будет удалено, очищено сборщиком мусора, в отличие от примера выше. Потому что все ссылки внутри функции больше не требуются после её отработки.
function deleteElement() {
  document.body.removeChild(document.getElementById('test'));
}
deleteElement();
