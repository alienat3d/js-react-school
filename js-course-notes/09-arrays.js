...
// Существуют ещё несколько методов для трансформации массивов через callback-функцию. Это filter(), map(), every(), some(), reduce(). Эти методы также называют «методами трансформации массивов», т.е. они берут исходный материал и его модифицируют. В то время, как forEach() просто перебирает.
// map() - вернёт новый массив, который будет отличаться от старого, потому что его элементы будут модифицированы.
// filter() - тоже самое, что и в map() происходит и здесь. Мы отфильтровываем массив по критерию, который зададим, например оставить только чётные числа. Или оставить элементы, тип данных у которых строка.

// Также есть методы split(), join(), sort().

// *Примеры*

// Представьте, что мы от пользователя или сервера получили большую строку, где через какой-то знак, например запятую, перечисленны товары. Нам требуется превратить их в обычный массив, где у нас товары будут лежать по порядку. Здесь нам поможет метод split().
// Переменная "str" будет получать ответ от пользователя.
// Всё, что введёт пользователь через запятую, станет элементом массива.
// На основании этой строки формируем список товаров products.
const str = prompt('', '');

const products = str.split(', '); // В скобках у метода split() указываем в кавычках какой будет разделитель у указанных элементов, например запятая.

console.log(products);

// Но есть и обратная операция, если мы хотим сделать наоборот, взять весь массив и создать из него строку.
// Это join() метод, который берёт каждый элемент нашего массива и склеивает их в одну строку.
// Внутри скобок join() можно указать разделитель между элементами в нашей строке, например ", ".
const newArray2 = ['Jack', 'Daniels', 'Joe', 'Morgan', 'Bob', 'Marley'],
  i = newArray2.join('; ');

console.log(i);

// А если вдруг нам понадобится отсортировать элементы в массиве по алфавиту, то мы используем метод sort()
const s = newArray2.sort();
...
const numbersArray = [1, 2, 15, 25, 333, 9],
...
const numbersArray2 = [1, 2, 15, 25, 333, 9],
...
/* 
  Ещё есть псевдо-массивы — это объект, структура, которая совпадает со структурой массива, он хранит элементы в индексах, но при этом псевдо-массивы не обладают методами, свойственными полноценным массивам из-за отличий в своём прототипе. У них нету всех этих методов, что мы выше разобрали. Например можно сформировать псевдо-массив с набором элементов с одинаковыми тегами, к примеру. 
*/
