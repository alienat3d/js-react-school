...
// ? Также следует помнить, что если промис fetch() попадает на ошибку HTTP-протокола, например Status: 404, например ошибка в названии URL и т.д. он не выкинет reject, он не посчитает это ошибкой и выполнит resolve. Ведь главное для fetch(), что он вообще смог сделать запрос. Поэтому reject будет возникать только при сбое сети и если что-то помешало запросу выполниться. В следующем уроке мы рассмотрим как заставить выкинуть reject, даже если запрос прошёл, но что-то пошло не так.
