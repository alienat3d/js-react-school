...
> Как вообще хук Реакта «useEffect» отслеживает изменения стейта?
...
- Реакт-хук «useState» хранит состояние (данные для UI), в то время, как реакт-хук «useRef» хранит ссылку на значение или DOM. При изменении стейта идёт ререндер родительского компонента, а в рефах такого не происходит. «useState» обновляется асинхронно (обычно), а «useRef» синхронно (мгновенно). Хук «useState» используется для всего, что мы видим на экране (текст, списки, переключатели), а хук «useRef» для технических данных (таймеры, предыдущие пропсы) и DOM-доступа.

> Что такое хоки и для чего их используют?
 
- "Хок" (HOC - Higher Order Component) — это компонент высшего порядка используется: во-первых, для оптимизации приложений путём сокращения кода и предотвращения его повторения, когда у очень похожих по логике компонентов, но с небольшими отличиями, включая вёрстку, выносится логика в HOC, а потом компоненты оборачивают этим хоком, а также вызывается функция добавляющая какие-то отличительные особенности. Во-вторых, они бывают полезны, когда нужно добавить какой-то новый функционал уже готовому компоненту, при этом не изменяя его внутренности напрямую.

> Как работает функция "bindActionCreators" в Redux?

- См. урок [\react-course-notes\8-192-bindActionCreators.md]

> Что значит "нормализация данных"? 

- Это приведение всех данных к одному формату / к одной структуре и при этом мы избавляемся от дублей.
