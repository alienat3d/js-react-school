  ...
  */
  ...
  const postData = form => {
    form.addEventListener('submit', evt => {
      ...
      request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); // 1.6.2 Далее нам нужно объект FormData перевести в JSON.
      ...
      request.send(json); // ? Есть ещё один нюанс, связанный с бэкенд-разработкой. PHP нативно не умеет с форматом данных JSON и чаще всего такие данные будут отправляться на сервера с использованием NodeJS. Но тем не менее есть возможность поработать и в PHP-окружении с таким типом данных. (см. далее в server.php файле)
