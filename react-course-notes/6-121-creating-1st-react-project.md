...
Итак, если перейти в папку [projects/react-project-1], то мы увидим всё также, как и раньше при работе с Webpack файл-точку входа index.js, в который импортирован App.js, в нём мы будем работать. Также там есть файл базовых CSS-стилей, а также файл тестирования и файл доп. функции WebVitals.
...
? Т.к. у нас здесь запущен Webpack, мы можем напрямую подключать файлы стилей.

Итак, функция ReactDOM запускается один раз на самом верхнем уровне приложения. Всё приложение мы будем собирать по отдельным файлам и подключать к App.js, который как конструктор по кусочкам будет собирать всё приложение и отдавать методом createRoot объекта ReactDOM на страницу, чтобы показать приложение.
