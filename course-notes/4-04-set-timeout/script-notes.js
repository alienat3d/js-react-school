...
// И в принципе setTimeout() внутри себя также может вызвать ещё один setTimeout(). Т.е. когда закончится внешний таймер запустится внутренний и т.д. И здесь мы встретимся с проблемой, что когда таймер с интервалом работает, он не учитывает то, как долго будет работать функция внутри него. Имеется в виду именно setInterval(). Ему всё равно сколько будет выполнятся функция внутри него. Но если так случится, что функция внутри будет тяжёлой и ресурсо-затратной и потребует больше времени на своё выполнение, чем таймер setInterval(), то он не будет ждать свой таймер, а будет думать, что его таймер уже прошёл во время выполнения функции и сразу запустит следующий вызов функции. Вот, чтобы таких казусов не случалось, то используют рекурсивный вызов setTimeout().
